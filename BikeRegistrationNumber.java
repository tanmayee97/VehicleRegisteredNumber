	...
	if(s.length()==10)
	{
		if(s.charAt(0)>=65 && s.charAt(0)<=90 && s.charAt(1)>=65 && s.charAt(1)<=90 &&
				...
				s.charAt(8)>=48 && s.charAt(8)<=57 && s.charAt(9)>=48 && s.charAt(9)<=57) 
		          {
					System.out.println("Valid Bike Registered Number");
				  }
					else
					{
						System.out.println("Enter Correct Bike Registered number");
					}
					}
	else
	{
	System.out.println("Enter Valid Bike Registered number");
}
